...
from habits.serializer import HabitSerializer, HabitCreateSerializer, HabitListSerializer, UserHabit
...
class HabitListAPIView(generics.ListAPIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        user_connected_public_habits = HabitConnection.objects.filter(user=user).values_list('habit', flat=True)
        connected_public_habits = Habit.objects.filter(id__in=user_connected_public_habits, is_public=True)
        connected_public_habits_serializer = HabitSerializer(connected_public_habits, many=True)
...
class HabitPublicConnectAPIView(generics.CreateAPIView):
    ...
    def perform_create(self, serializer):
        ...
        habit_connect = HabitConnection.objects.filter(user=self.request.user, habit=habit_id)
